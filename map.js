...
class Map {
  constructor(tiles) {
    this._tiles = tiles;
    this._items = {};
    this._entities = {}; // Create the Scheduler and Engine
    ...
    for (xTile in this._tiles) {
      for (yTile in this._tiles[xTile]) {
        if (this._tiles[xTile][yTile].isSolid()) {
          if (!this.adjacentToUnsolidTile(xTile, yTile)) this._tiles[xTile][yTile].setUnreachable();
        }
      }
    }
  }
  ...
  addEntityAt(x, y, entity) {
    // Add map to entity
    entity.setMap(this); // check if tile is empty
    // Check if x and y are in bounds
    // Set entities position

    entity.setX(x);
    entity.setY(y); // Add entity to the map

    this._entities[entity.getUid()] = entity; // Add our entity to the map scheduler
  ...
  addItemAt(x, y, item) {
    item.setX(x);
    item.setY(y); // Add Item to the map
