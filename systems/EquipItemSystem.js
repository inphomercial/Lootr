...
const EquipItemSystem = (entity, item) => {
  ...
  if (!item.hasComponent('Wieldable')) {
    LoggerPlayer("You cannot figure out a way to equip the ".concat(item.getName()));
    return;
  }
  ...
  LoggerPlayer("You equip the ".concat(item.getName()));
};
...
const UnequipItemSystem = (entity, item) => {
  ...
  LoggerPlayer("You un-equip the ".concat(item.getName()));
};