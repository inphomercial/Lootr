...
function MoveableSystem(entity, dX, dY) {
  ...
  if (!entity.hasComponent('Moveable')) {
    LoggerPlayer("".concat(entity.getName(), " is unable to move"));
    return false;
  }

  if (newX < 0 || newY < 0 || newX >= map.getWidth() || newY >= map.getHeight()) {
    LoggerPlayer("".concat(entity.getName(), " cannot move off the map"));
    return false;
  ...
  if (map.isTileOccupied(newX, newY)) {
    ...
    LoggerPlayer("".concat(entity.getName(), " tries to move into ").concat(newX, ", ").concat(newY, " but there is a ").concat(occupiedBy.getName(), " there.")); //TODO Don't make enemies attack eachother when they are in the way.
  ...
  if (itemInLocation) {
    LoggerPlayer("You see a ".concat(itemInLocation.getName(), " on the ground."));
  }
...
const getPathToCoords = (entity, coords, map = false) => {
  ...
  const isPassable = function isPassable(x, y) {
    if (x < 0 || y < 0) {
      return false;
