...
const AbilityCheckSystem = (entity, stat, challengeNumber, hasAdvantage = false, hasDisadvantage = false) => {
  ...
  let roll; // If has advantage or disavantage (roll the d20 twice)
  ...
  const statString = "get".concat(stat);
  const entityStat = Lootr.EntitySystems.Stats[statString](entity); // get modifier score for the stat

  const modifier = Lootr.EntitySystems.Stats.getModiferScore(entityStat); // apply bonuses + modifer to roll

  const total = roll + modifier; // compare against total to a target number
