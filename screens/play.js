...
Lootr.Screens.Play = {
  ...
  renderGame: function renderGame() {
    Lootr.getGameDisplay().clear(); // Draw map first
    ...
    this.map.renderEntities(this._gameDisplay, Lootr.getPlayer().getCoordinates()); // Draw Player Stats
  ...
  handleInput: function handleInput(inputType, inputData) {
    ...
    switch (code) {
      ...
      default:
        LoggerPlayer("Unkown Command ".concat(code));
        return;
    }
